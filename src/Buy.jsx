      ...
      <div className="popup-content">
        ...
        <div className="size-options">
          {sizes.map((size, index) => (
            <button key={index} className="size-option" onClick={() => window.open(`https://wa.me/+33766197937/?text=Salut, j’aimerais bien acheter la photo numéro ${index+1}, ${size.size}!`, "_blank")}>
              <h3>{size.name}</h3>
              <p>{size.size}</p>
